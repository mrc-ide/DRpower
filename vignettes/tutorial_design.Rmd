...
We can see that at least 5 clusters are needed, otherwise sample sizes are too large to be reported in this table. It's also interesting to note that the total sample size (`n_clust * N_opt`) *decreases* as we recruit more clusters. For example, a 6-cluster design would require 678 samples in total, but a 10-cluster design would require only 300. This is one of *many* arguments for recruiting as many clusters as possible.

Let's assume that we can only recruit 6 clusters for logistical reasons, meaning we are aiming for 113 samples per cluster. These clusters can be chosen through randomisation of all health facilities, or by using predesignated sentinel surveillance sites. In the latter case, it is important that sentinel sites are chosen to be representative of the population in the domain as a whole (e.g. in terms of demographic and social characteristics), otherwise we risk getting a biased view of the prevalence of *pfhrp2/3* deletions.
...
```{r}
# plot power as a function of per-cluster sample size
plot_power(n_clust = 6, prevalence = 0.1, N_max = 200)
```

We find that the power curve is fairly shallow around the chosen value of N = 113. For example, the value N = 70 still achieves 75% power. We need to be pragmatic about these numbers, and not stick dogmatically to the target value of 80% power without thinking about what it means. We should remember that power is defined as the probability of finding an interesting result if it is there, so a study with 75% power has alsmost as good a chance of success as a study with 80% power. It is certainly not the case that if we fail to hit 80% then the study is completely invalid. This is important because a drop in sample size from 113 per cluster to 70 per cluster may be the difference between a study that is feasible to conduct and one that is not. We need to balance these factors against each other when coming up with a study design.

That being said, we should not allow power to drop too low or the study may not be worth doing. In our case we will stick to the target value of 113 samples per cluster, as we believe we can raise funds to this level and successfully carry out the study with this sample size.

## 3. Refine cluster sizes

Next, we should examine our 6 clusters individually to see if they are able to recruit 113 samples. Ideally, this should be informed by historical incidence trends at each of the sites, which can give a good indication of how many malaria cases tend to be seen throughout the transmission season.
...
The question is - how badly do these smaller cluster sizes hurt our power? We cannot work this out from sample size tables or power curves because these assume the same sample size in each site. But we can explore power directly using the `get_power()` function:

```{r, echo=FALSE}
set.seed(1)
```
```{r}
get_power(N = c(113, 113, 60, 60, 40, 40),   # new cluster sizes
          prevalence = 0.1,
          ICC = 0.05,
          prev_thresh = 0.05,
          reps = 1e2)
...
```
```{r}
get_power(N = c(113, 113, 60, 60, 40, 40, 40),   # one more cluster of size 40
          prevalence = 0.1,
          ICC = 0.05,
          prev_thresh = 0.05,
          reps = 1e2)
...
We often plan on using the same samples for other research purposes. For example, it is common to look for drug resistance markers at the same time as *pfhrp2/3* deletions. In this case, we should perform a separate power analysis and sample size calculation for these other end-points and always *take the largest sample size* between the two analyses, as this is the one that will give sufficient power over all end-points.

The *DRpower* package contains simple functions for exploring other questions. For example, imagine we are looking to detect the presence of rare variants, for example *pfk13* mutations conferring partial resistance to artemisinin. Here, we are asking whether there are *any* mutants in the domain, and a single positive result confirms that there are. The function `get_power_presence()` tells us our power to detect *any* mutants given the known prevalence. Similar to the *pfhrp2/3* deletion analysis, this assumes we will run a multi-site study and summarise results over all sites, meaning we need to account for intra-cluster correlation:
...
So six clusters of 58 samples would do it. We have variable sample sizes, some above this level and some below, which is why the `get_power_presence()` method was useful.

What if we are not interested in summarising at the domain level, and instead we want to know our power in each of the individual sites? We can use the same functions as before, now run independently over each of the clusters. We should fix the ICC at zero because this is no longer a multi-cluster study:

```{r}
# how many
get_sample_size_presence(n_clust = 1, prevalence = 0.01, ICC = 0)

# get power to detect a 1% variant in each of the individual clusters
...
What if we also plan on using the same samples to look for a drug resistance marker that we believe to be at high prevalence? In this case, it can be useful to consider what will be the expected margin of error in our analysis. This is not the same as a power analysis, as we are not conducting a hypothesis test. Rather, we are trying to choose a sample size that gives us a certain level of precision in our analysis.

The `get_sample_size_margin()` function tells us the per-cluster sample size needed to achieve a given margin of error (MOE) if we plan on pooling results over multiple clusters. Here, we assume that the prevalence of the marker is 20% in the population, and we hope to estimate this to within plus or minus 10%:

```{r}
# get sample size needed to achieve 10% MOE if prevalence is 20% and we have 6 clusters
...
```{r}
# get MOE assuming 40 samples in each of 6 clusters
get_margin(N = 40, n_clust = 6, prevalence = 0.2, ICC = 0.05)
```

With 40 samples we would expect to estimate prevalence to within plus or minus 8.7%.

The formula used above is extremely simple. It does not allow for different sample sizes per cluster, and it also assumes we know the ICC exactly rather than estimating it from data. An alternative approach is to use the *DRpower* Bayesian model to estimate the prevalence. In other words, we will use the `get_prevalence()` function to obtain a 95% Bayesian credible interval (CrI). If we plan to estimate prevalence this way, then we should use the function `get_margin_Bayesian()` to estimate our MOE. This function works by simulation, and accepts different sample sizes in each cluster:
...
Here, we expect around 13% error on the lower end, and around 31% error on the upper end. In other words, if the true prevalence of the marker is 20% in the population then we would expect our 95% CrI to extend from around 7% to 51%. This is quite a wide interval, and much wider than would assume based on the simpler `get_margin()` function. The reason is that the Bayesian model is making different assumptions, most notably estimating the ICC from data rather than assuming a known value.

In summary...

- We have sample sizes (`N`) that we are happy with for our *pfhrp2/3* analysis. These don't quite achieve 80% power, but they are not far off. They have been chosen taking into account logistical and feasibility constraints. We also know the number of suspected malaria cases that we need to enrol (`N_suspected`).
- The sample sample sizes give us 86% power to detect rare *pfk13* variants at 1% in the population when summarised at the domain level. We do not have power at the individual site level, meaning there is a good chance we would miss some mutants in the smaller sites if present at 1% prevalence.
- The sample sample sizes give us a margin of error of around 13% on the low side and 31% on the high side when estimating the prevalence of a variant at 20% in the population. This is a fairly low degree of precision, and so we should only expect to be able to conclude roughly which quarter of the prevalence range our marker is in.

Notice that this summary considers all end-points for the same sample sizes. We have now arrived at a detailed plan, and have a good idea of what we can expect from our results if we go ahead with these numbers.
