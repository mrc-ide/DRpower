...
The `get_prevalence()` function calculates the full posterior distribution of the prevalence, but then it usually returns just a summary of this distribution (unless `post_full_on = TRUE`). There are many summaries that we could choose, for example, we might choose the posterior mean. But how good a summary is this? The plot below shows the posterior mean for the 6-cluster case compared with the true prevalence used in simulation, averaged over 1000 simulations. If the method is unbiased then the coloured lines should match the 1:1 line (dashed):
