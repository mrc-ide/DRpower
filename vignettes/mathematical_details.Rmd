...
rbind.data.frame(list(Parameter = "$p$", Definition = "Prevalence at the province level. This is usually the main thing we are interested in estimating")) %>%
  ...
  scroll_box(width = "500px", height = "360px")
```
...
Data, in the form of counts of observed *pfhrp2/3* deletions, are then assumed to be binomial within each cluster, drawn from the cluster-level prevalence. This results in counts following a beta-binomial distribution, with the following probability distribution:
...
This integration is performed by a numerical quadrature-based method. The marginal posterior distributions can be returned in full, or can be summarised using various measures of central tendency and/or credible intervals (CrI). There are multiple methods of calculating CrIs - we use the high density region (HDI) by default because this avoids the possibility of the maximum a posteriori (MAP) estimate being outside the CrI, which is possible by the more commonly used equal-tailed interval.
