...
vignette: >
  %\VignetteIndexEntry{rationale3_design_effect}
...
We have defined both of these variances above when talking about CIs. The variance under the cluster-level analyis is given by $var(\hat{p}_c) = \frac{s^2}{c}$, and the variance under SRS is given by $var(\hat{p}_s) = \frac{\hat{p}(1 - \hat{p})}{nc}$. For our example data this gives $D_{eff} =$ `r round(Deff, 2)`. This is a fairly large design effect, meaning there is a large impact of overdispersion between clusters that is creating statistical inefficiency.

### 3.1 The effective sample size

Another way to think about this is in terms of the *effective sample size* (ESS). This is the number of effectively independent samples we have in our study, which will be smaller than the actual sample size whenever there is intra-cluster correlation. ESS is simply calculated as the true sample size divided by the design effect: $n_E = \frac{nc}{D_{eff}}$. In our case we find that $n_E =$ `r n*c` $/$ `r round(Deff, 2)` $=$ `r round(Neff, 2)`, which we would round down to `r floor(Neff)` to be conservative. Some people (myself included) find this to be a very useful way of thinking about statistical efficiency. Even though we went to all the trouble of sampling `r n*c` individuals, the fact that people are so similar within clusters means that effectively we only sampled `r floor(Neff)` distinct individuals in total. This is a very "real world" way of thinking about something that can otherwise feel quite abstract.

Now that we have an estimate of the sample size that takes account of correlations we could use it within our original CI formula in place of the true sample size. Using $n_E$ in place of $nc$ we obtain the interval $\hat{p} \pm z\sqrt{\frac{\hat{p}(1 - \hat{p})}{n_E}}$. At first glance this appears to give us *yet another* way of constructing CIs, but don't be fooled. Once you substitute in the formula for $n_E$ and trace it back to the variance you'll find that this simplifies to $\hat{p} \pm z\sqrt{\frac{s^2}{c}}$, which was our formula from the second CI method above. So, the design effect doesn't give us a magic way of dealing with the issue of ICC, it just gives us a different way of thinking about it. Rather than taking a whole new statistical approach it's like we use the original approach but we "punish" the method for the inefficient design by stretching out the CI. Either way you choose to look at it, you get the same result.

### 3.2 Relating $D_{eff}$ to the ICC

One more important point to make is that $D_{eff}$ can be related to the ICC (denoted $r$ hereafter) through the formula:

$D_{eff} = 1 + (n - 1)r$

Notice that when $r = 1$ (perfect correlation within clusters) we find that $D_{eff}$ reaches its maximum value of $n$, and the effective sample size crashes down to $n_E = \frac{nc}{n} = c$. This should make some intuitive sense - when there is perfect correlation we effectively have one observation per cluster because all subsequent observations will be dictated by the first.

But this formula also reveals one of the tricky things about $D_{eff}$, which is that it only applies for a particular sample size. From the formula above we can see that a clustered prevalence survey with twice the sample size (i.e., twice the $n$) will have roughly twice the $D_{eff}$, making it difficult to compare values between studies. On the other hand, the parameter $r$ does not change with sample size, and so is more of an *intrinsic* property of the population. For this reason, we tend to talk in terms of the ICC inside the DRpower package.
