...
df_filtered %>%
  ggplot() + theme_bw() +
  geom_pointrange(aes(x = N, y = power, ymin = lower, ymax = upper, colour = "DRpower")) +
...
There is some random noise in this plot, but the overall trend is clear. Power increases quickly at first and then plateaus out for larger sample sizes. Power reaches 80% for the DRpower method at around 113 samples per cluster. In contrast, the z-test method never reaches the 80% power level.
...
2. Intuitively, we would expect per-cluster sample sizes to decrease as the number of clusters increases. In some rare cases this method can give the opposite result due to random fluctuations. We therefore remove this effect by manually modifying results so that sample size can never decrease with more clusters.
...
The best way to perform all of these steps is through the [interactive web app](https://shiny.dide.ic.ac.uk/DRpower-app/). But for those wanting to perform the same analysis in the R package, see [this tutorial](articles/tutorial_design.html).
