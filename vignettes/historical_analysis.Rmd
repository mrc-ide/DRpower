...
5. **Merge per site**. Combined counts (tested and positive) of studies conducted in the same exact location (based on lat/lon) in the same year and from the same source publication. These are considered a single site.
6. **Number of samples**. Filtered to have 10 or more samples per site. Avoids very small sample sizes which would have very little information from the data and therefore would be driven by our prior assumptions.
7. **Map to ADMIN1**. All sites were mapped to ADMIN1 level by comparing the lat/lon coordinates against a shapefile from [GADM](https://gadm.org/) version 4.1.0, first administrative unit.
...
9. **Filter on number of sites**. Filter to ADMIN1 regions that contain at least 3 distinct sites
within the same year and from the same source publication.

This filtered the original data down to 6 studies and 7 ADMIN1 domains globally. This filtered dataset is available within the package through the `historical_data` object:
...
The second place we need to know the ICC is when estimating power. In our simulation approach, we are forced to simulate data under an assumed ICC value. For the reasons above, we focus on the case of ICC = 0.05 as a realistic value that is likely to hold true for most studies. However, if the aim is to be cautious about the ICC then one can opt for a larger value, which will in turn lead to larger sample sizes.
