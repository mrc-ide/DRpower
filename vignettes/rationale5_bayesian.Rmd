...
In DRpower we take a different approach to analysis that deals with all of the issues listed in the previous section. We describe the approach at a high level here, for those wanting more mathematical details please [see this page](articles/mathematical_details.html).
...
The first output is the *maximum a posteriori* (MAP) estimate, which gives us a point estimate of `r gp$MAP`% prevalence - very close to our original estimate of `r round(p*100, 2)`% from the raw data. There are other summaries that we might be interested in, but see [here](articles/summarise_prevalence.html) when deciding between them.
