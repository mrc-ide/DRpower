...
- **CIs are symmetric**. The CI formula in section 2.1 is based on the central limit theorem, which argues that the sampling distribution of the statistic is approximately normal when the number of clusters is large and when the true prevalence is somewhere near 50%. In our case, both of these assumptions tend to be violated - the number of clusters can be *very* small and prevalence tends to be close to 5%. One consequence is that CIs are forced to be symmetric when in fact they should often be asymmetric and typically thinner on the side of small values. This makes us more likely to fail to reject the 5% threshold when in fact there is strong enough evidence in the data to reject it.

- **Error is not propagated**. The CI formula in section 2.1 makes use of the sample variance as a fixed value, but this statistic will also be subject to random variation and so should estimated with uncertainty. Ideally, this uncertainty would be propagated through to our CI on the prevalence, but this is not done here. Normally we could switch to a method like [bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)) to propagate error, but this is also not appropriate here as it relies on a large number of observations (i.e. clusters).
...
- **Shouldn't this be a hypothesis test?** We have assumed so far that we will construct CIs and then accept or reject the 5% threshold based on this CI. But this confuses estimation with hypothesis testing, when in fact there may be some advantages to keeping them separate. For example, one could argue that we need a one-sided hypothesis test as we only really care if prevalence is *above* this threshold, while at the same time we want a two-tailed CI for estimating the prevalence. Mixing these concepts means we may have lower statistical power than we could achieve.

Sometimes these issues will be very minor, and sometimes they may even cancel each other out, but *sometimes they won't*! We need to be confident that our analysis plan is robust no matter what our data end up looking like, hence the need for the DRpower model.
