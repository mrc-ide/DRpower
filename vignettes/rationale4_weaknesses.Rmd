...
vignette: >
  %\VignetteIndexEntry{rationale4_weaknesses}
...
## 4. Weaknesses with the traditional approach
...
- **CIs are symmetric**. The CI formula above is based on the central limit theorem, which argues that the sampling distribution of the statistic is approximately normal. But this approximation is only accurate when the number of clusters is large and when the true prevalence is somewhere near 50%. In our case both of these assumptions tend to be violated - the number of clusters can be *very* small and prevalence tends to be close to 5%. One consequence is that CIs are forced to be symmetric when in fact they should often be asymmetric and typically thinner on the side of small values. This makes us more likely to fail to reject the 5% threshold when in fact there is strong enough evidence in the data to reject it.

- **Error is not propagated**. The CI formula makes use of the sample variance as a fixed value, but this statistic will also be subject to random variation and so should estimated with uncertainty. Ideally, this uncertainty would be propagated through to our CI on the prevalence, but this is not done here. Normally we could switch to a method like [bootstrapping](https://en.wikipedia.org/wiki/Bootstrapping_(statistics)) for calculating CIs, but this is also not appropriate here as it relies on a large number of observations (i.e. clusters) which is not true in general.

- **Cannot deal with 0 counts**. It is quite common in historical *pfhrp2/3* datasets to see zero deletions in some clusters, and even sometimes in the study as a whole. This is to be expected when we looking for relatively low prevalence of deletions. If there are zero deletions in the study then the sample variance will also equal zero, and so the CI is centred at 0% prevalence and has 0 width. This gives the false impression that we are extremely confident that prevalence is 0%, when in fact this is just an artefact of our analysis. This is particularly problematic when the sample size is very small, as here we are even more likely to see zero deletions, creating the uncomfortable situation that we are most likely to confidently conclude 0% prevalence when we have the least evidence to support this claim!
...
- **Shouldn't this be a hypothesis test?** We have assumed above that we will construct CIs and then accept or reject the 5% threshold based on this CI. But this confuses prevalence estimation with hypothesis testing, when in fact there may be some advantages to keeping them separate. For example, one could argue that we need a one-sided hypothesis test as we only really care if prevalence is *above* this threshold, while at the same time we want a two-tailed CI for estimating the prevalence. Mixing these concepts means we may have lower statistical power than ideal.
