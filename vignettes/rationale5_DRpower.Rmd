...
In DRpower we take a different approach to analysis that deals with all of the issues listed in the previous section. We only describe the approach at a high level here, for those wanting more mathematical details please [see this page](TODO).
...
What we end up with is a probability distribution that describes the plausible range of values that $p$ could take. We can use this in many different ways, for example we can calculate credible intervals (CrI). CrIs have a slightly different and more direct interpretation than CIs - a 95% CrI means there is a 95% probability that the true value lies inside this interval (compare this with [the definition of a CI](https://thestatsgeek.com/2020/11/21/interpretation-of-frequentist-confidence-intervals-and-bayesian-credible-intervals/)). This immediately deals with the first 4 out of 5 issues listed in the previous section.

We can also output from the model the posterior probability that prevalence is above the 5% threshold. We can turn this into a hypothesis test by rejecting the null hypothesis whenever this probability is above a certain level (0.95 by default). This gives us a binary decision tool just like the traditional approach, but now with all the advantages of the Bayesian method.
...
We also estimate that there is a `r gp$prob_above_threshold` probability that the prevalence is above 5%. This probability is above 0.95, meaning we can reject the null hypothesis. Based on this analysis we have sufficient evidence to conclude that prevalence is above our target threshold, and therefore  this warrants a change in RDT use.
