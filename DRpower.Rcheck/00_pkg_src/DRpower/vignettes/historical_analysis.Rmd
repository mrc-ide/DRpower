...
This filtered us down to 12 studies. Granular data from these 12 studies is available within the package through the `historical_data` object. At this point, a further filter was applied:

1. **ADMIN1 units must contain data from 3 or more sites within the same sampling year**. Multiple sites are needed to provide information on the ICC, and 3 sites was chosen as a minimum as this is more informative than e.g. 2 sites while still providing adequate data.
...
The other place we need ICC estimates is when estimating power, as we are forced to simulate under an assumed ICC. For the reasons above, we focus on the case of ICC = 0.05 as this is a realistic value that is likely to hold true for most studies. However, if we want to be cautious about the ICC then we can instead opt for a larger value, which will in turn lead to larger sample sizes.
